...
def allPlot(P, U, C, E, timeSteps, MC, NW, eulerH, wbcNeutrophil, wbcLymphocyte, wbcNeutrophilMin, wbcLymphocyteMin):
    ...
    for e in range(len(P)): 
        ...
        plt.savefig('Pop_scenario'+str(e+1)+'.png')
        plt.close()
    #
    for e in range(len(P)):
        for q in range(len(P[0])):
        ...
        plt.savefig('logPop_scenario'+str(e+1)+'.png')
        plt.close()
    #
    for d in range(len(U)):
...
def sto_Chemo(treatmentLength, alpha, eulerH, scenarios, popTypes, drugTypes, bioCons, vol, gompParam, popMaxes, initPops, scenarioProb, surgerySize, effectCoeffs, effectExpons, opParams, pillRegularize, _gap):      
    # 
    theModel = Model()
    ...
    #
    theModel.setObjective(sum(P[0][q][len(stageSteps) - 1] for q in popTypes), GRB.MINIMIZE)
    #
...
def run_sto_Chemo(**kwargs):                      # may input GAP for debugging purposes, e.g., gap=0.005 for 0.5% gap -- no input equals no gap
    ...
    if 'gap' in kwargs:
        ...
        _gap = 0.0
    output = sto_Chemo(treatmentLength, alpha, eulerH, scenarios, popTypes, drugTypes, bioCons, vol, gompParam, popMaxes, initPops, scenarioProb, surgerySize, effectCoeffs, effectExpons, opParams, pillRegularize, _gap)
